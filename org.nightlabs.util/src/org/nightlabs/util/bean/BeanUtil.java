...
/**
 * @author unassigned (Original author)
...
public class BeanUtil
{
	...
	/**
	 * @param _propertyDescriptors A List of PropertyDescriptors
	 * @return A List of PropertyEditorUIs for the Parameter _propertyDescriptors
	 *
	 * @see java.beans.PropertyDescriptor
	 * @see org.nightlabs.editor2d.shared.drawcomponents.propertyeditors.PropertyEditorUI
	 */
	public List<PropertyEditorUI> getPropertyEditorUIsFromPDs(List<PropertyDescriptor> _propertyDescriptors)
	{
		Vector<PropertyEditorUI> editorUIs = new Vector<PropertyEditorUI>();
		for (Iterator<PropertyDescriptor> it = _propertyDescriptors.iterator(); it.hasNext(); )
		{
	...
	/**
	 * This Method returns all PropertyEditorUIs for a certain amount of beans
	 *
	 * @param _beans A List of targetBeans
	 * @return A List of PropertyEditorUIs which all target beans of the
	 * Parameter _beans share
	 * @throws IntrospectionException
	 */
	public List<PropertyEditorUI> getPropertyEditorUIsFromBeans(List<? extends Object> _beans)
	throws IntrospectionException
	{
		Class<? extends Object> classForAllBeans = getAllBeansSingleClass(_beans);
		if (classForAllBeans != null)
		{
			List<PropertyDescriptor> pds = getPropertyDescriptorsAsVector(classForAllBeans, getCurrentMode());
			List<PropertyEditorUI> editorUIs = getPropertyEditorUIsFromPDs(pds);
			return editorUIs;
		}
		else {
			List<PropertyDescriptor> pds = getSamePropertyDescriptors(_beans);
			List<PropertyEditorUI> editorUIs = getPropertyEditorUIsFromPDs(pds);
			return editorUIs;
		}
	}
	 ...
	 */
	public void loadUIValues(List<PropertyEditorUI> _editorUIs, List<? extends Object> _beans)
	throws PropertyEditorUIException
	{
		for (Iterator<PropertyEditorUI> it = _editorUIs.iterator(); it.hasNext(); )
		{
			PropertyEditorUI editorUI = it.next();
	 ...
	 * Sets the UI components with values reflecting the beans state
	 */
	public void loadUIValues(List<PropertyEditorUI> _editorUIs, Object _bean)
	throws PropertyEditorUIException
	{
		for (Iterator<PropertyEditorUI> it = _editorUIs.iterator(); it.hasNext(); )
		{
	...
	/**
	 * Save the value displayed by the UI components into all
	 * beans in the beans Vector
	 */
	public void saveUIValues(List<PropertyEditorUI> _editorUIs, List<? extends Object> _beans)
	throws PropertyEditorUIException
	{
		for (Iterator<PropertyEditorUI> it = _editorUIs.iterator(); it.hasNext(); )
		{
	...
	/**
	 * Save the value displayed by the UI components into
	 * the single bean Object
	 */
	public void saveUIValues(List<PropertyEditorUI> _editorUIs, Object _bean)
	throws PropertyEditorUIException
	{
		for (Iterator<PropertyEditorUI> it = _editorUIs.iterator(); it.hasNext(); )
		{
	...
	/**
	 * @param _propertyName The name of the property
	 * @param _beans A List of target beans
	 * @return A Map with a bean as key and the corresponding
	 * 				 PropertyEditorUI as value
	 * @throws IntrospectionException
	 */
	public Map<Object, PropertyEditorUI> getPropertyEditorUI(String _propertyName, List<? extends Object> _beans)
	throws IntrospectionException
	{
		Map<Object, PropertyEditorUI> bean2EditorUI = new HashMap<Object, PropertyEditorUI>();
		for (Iterator<? extends Object> itBean = _beans.iterator(); itBean.hasNext(); )
		{
			Object bean = itBean.next();
			PropertyDescriptor[] pds = getPropertyDescriptors(bean.getClass(), currentMode);
